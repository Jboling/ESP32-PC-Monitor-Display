...
static void example_lvgl_flush_cb(lv_display_t * disp, const lv_area_t * area, uint8_t * color_p)
{
  ...
  /* Manual full-frame rotation (Waveshare V8) */
  uint32_t index = 0;
  uint16_t *data_ptr = (uint16_t *)color_p;
  uint16_t *rotat_ptr = (uint16_t *)lvgl_dest;
  for (uint16_t j = 0; j < EXAMPLE_LCD_H_RES; j++) {
    for (uint16_t i = 0; i < EXAMPLE_LCD_V_RES; i++) {
      rotat_ptr[index++] = data_ptr[EXAMPLE_LCD_H_RES * i + (EXAMPLE_LCD_H_RES - 1 - j)];
    }
  }
#endif
